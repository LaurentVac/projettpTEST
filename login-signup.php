...
?>
   ...
   <!-- <a href="userpage.php"></a> -->
            ...
            <!-- fin login -->
            <!-- début sign up -->
            <form class="col-md-5 login mt-3 pb-3 ml-md-5 "action="" method="POST">
                <h3>Inscrivez-vous</h3> 
                <!-- Champ nom -->
                <div class="form-group">
                    <label for="name">Nom</label>
                    <input type="text" class="form-control" id="name" name="name" aria-describedby="lastname" pattern="[a-zA-ZàâæçéèêëîïôœùûüÿÀÂÆÇnÉÈÊËÎÏÔŒÙÛÜŸ-_]+" value="<?=$name ?? ''?>" required>
                ...
                <div class="form-group">
                    ...
                    <input type="password" class="form-control" id="password" name="password" pattern="^(?=.{10,}$)(?=(?:.*?[A-Z]){2})(?=.*?[a-z])(?=(?:.*?[0-9]){2}).*$" title='Le mot de passe doit contenir au moins 10 caractères dont 2 majuscules, 1minuscule et 2 chiffres. Les caractères spéciaux ne sont pas autorisés' value="<?=$password ?? ''?>" required>
                    <div id="password_error" class="form-text formError"><?= $errorsArray['password_error'] ?? ''?></div>
                </div>
                <!-- confirmation mot de passe -->
                <div class="form-group">
                    <label for="confirmPassword">Confirmez votre mot de passe</label>
                    <input type="password" class="form-control" id="confirmPassword" name="confirmPassword" pattern="^(?=.{10,}$)(?=(?:.*?[A-Z]){2})(?=.*?[a-z])(?=(?:.*?[0-9]){2}).*$" title='Le mot de passe doit contenir au moins 10 caractères dont 2 majuscules, 1minuscule et 2 chiffres. Les caractères spéciaux ne sont pas autorisés'value="<?=$confirmPassword ?? ''?>" required>
    ...
    <!--fin main-->
    <!-- début footer -->
<?php var_dump($array) include('views/templates/footer.php');?>